...
def get_nearest_neighbors(E, estimate_vector): #modified function used heap to reduce the time complexity to O(nlogk), k is fixed and is equal to 6 and hence complexity becomes O(nlog6) = O(n)    
    distance_map = [((E - float(val)) ** 2, idx) for idx, val in enumerate(estimate_vector)]
    smallest_distances = heapq.nsmallest(6, distance_map)
    return [idx for dist, idx in smallest_distances]
